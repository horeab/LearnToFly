...
public class RocketCreator extends SensorAccesoryCreator {
	...
	public RocketCreator(World world, Body playerBody, Integer accesoryLevel, AssetManager assetManager) {
		...
		if (hasAccesory()) {
			...
			Texture fireTextureWithFilter = Utils.getTextureWithFilter(assetManager, rocketfire);
			int fireWidth = fireTextureWithFilter.getWidth();
			int fireheight = fireTextureWithFilter.getHeight();
			TextureAnimation fire = new TextureAnimation(rocketfire, "rocketFire", Arrays.asList(0, 1), fireWidth / 2, fireheight, assetManager);

...
public class HUD {
	...
	public HUD(Player player, PlayerAttrs playerAttrs, AssetManager assetManager, LibgdxControlUtils libgdxControlUtils) {
		initFont(assetManager);
		this.displayWidth = Game.getWidth();
		this.displayHeight = Game.getHeight();
		createFinishPopup = new CreateFinishPopup(font, playerAttrs, assetManager);
		createAchievmentPopup = new CreateAchievmentPopup(font, assetManager, libgdxControlUtils);
		fuelBarFullTexture = Utils.getTextureWithFilter(assetManager, Resource.fuel_bar_full);
		fuelBarEmptyBackground = new TextureRegion(Utils.getTextureWithFilter(assetManager, Resource.fuel_bar_empty), 0, 0, 8, SPEEDOMETER_HEIGHT);
		speedometer = new TextureRegion(Utils.getTexture(assetManager, Resource.speedometer), 0, 0, SPEEDOMETER_WIDTH * 2, SPEEDOMETER_HEIGHT * 2);
		Texture speedNeedleTexture = Utils.getTexture(assetManager, Resource.speed_needle);
		speedNeedle = new TextureRegion(speedNeedleTexture, 0, 0, SPEEDNEEDLE_WIDTH * 2, SPEEDNEEDLE_HEIGHT * 2);
		this.player = player;
	}

	private void initFont(AssetManager assetManager) {
		Texture nr = Utils.getTextureWithFilter(assetManager, Resource.numbers);
		font = new TextureRegion[62];
		for (int i = 0; i < 10; i++) {
			font[i] = new TextureRegion(nr, i * NUMBER_CHARACTER_WIDTH, 0, NUMBER_CHARACTER_WIDTH, 40);
		}
		Texture upper = Utils.getTextureWithFilter(assetManager, Resource.alphabet_upper);
		for (int i = 10; i < 36; i++) {
			font[i] = new TextureRegion(upper, (i - 10) * ALPHA_UPPER_CHARACTER_WIDTH, 0, ALPHA_UPPER_CHARACTER_WIDTH, 40);
		}
		Texture lower = Utils.getTextureWithFilter(assetManager, Resource.alphabet_lower);
		for (int i = 36; i < 62; i++) {
			font[i] = new TextureRegion(lower, (i - 10 - 26) * ALPHA_LOWER_CHARACTER_WIDTH, 0, ALPHA_LOWER_CHARACTER_WIDTH, 40);
		}
		Texture greenNr = Utils.getTextureWithFilter(assetManager, Resource.green_numbers);
